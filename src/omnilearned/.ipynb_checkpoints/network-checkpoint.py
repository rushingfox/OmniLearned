...
class PET2(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.mode = mode
        if self.mode not in ["classifier", "generator", "pretrain", "ftag", "regressor"]:
...
class PET_regressor(nn.Module):
    def __init__(
        ...
        self.out = nn.Linear(self.num_tokens * base_dim, 1)
